...
import {  queryGoodsData , getCart , resetSearchReduces } from '../../common/actions/searchAction';
...
class Search extends baseComponent {
	...
	headerRightRender (){
		return (
			<Link to = {{ pathname : "/SearchKeyWords" }} style = { styles.searchBox } >
	...
	loadMoreData (){
		...
		if (!this.state.arrowTransform) {	//当显示品牌的时候下拉到底不请求数据
			if (this.state.totalPage >= index ) {
				const keyWords = this.props.location.query.keyWords || "";
				const brandId = this.props.location.query.brandId || "";
				this.props.queryGoodsData(index,10,this.state.sortBy,brandId,keyWords);
			}else{
				return false;
			}
		};
	...
	headerRender() {
		const self = this;
		let headerTitle = this.props.location.query.keyWords ? this.props.location.query.keyWords : 
			(this.props.location.query.categoryName ? this.props.location.query.categoryName : this.props.sendMessage);
		return(
			<div>
	...
	hasheader (){
		const self = this;
		var param ;
		param = self.props.getGoodsReducer === undefined ? "" : self.props.getGoodsReducer.data.goods;
		if (param.length > 0) {
				...
				<div style = { this.state.arrowTransform ? styles.hide : styles.show } >
				    ...
				    <div style = { styles.cart } >
				    	<div  style = { styles.cartBox } >
				    		<Link to = {{ pathname : "/CartPage" }} >
					    		<img style = { styles.cartImg } src = "common/images/i-cart@3x.png" />
					    		<p style = { styles.cartNum } >{ this.props.cartReducer.data }</p>
					    	</Link>
					    </div>
				    </div>
			    </div>
	...
	render () {
		const { queryGoodsData , getCart ,brandReducer, getGoodsReducer ,searchParam , cartReducer , resetSearchReduces } = this.props;
		var headerParam = {
	...
	allGoods (){
		...
		if (goods) {
			...
			if (flag) {
				this.state.goodsRender = goodsItems;
			}else{
				if (this.state.goodsRender.length == 0 ) {
					this.state.goodsRender = goodsItems;
				};
				this.state.goodsRender = this.state.goodsRender.concat(goodsItems)
				this.state.goodsRender = this.unique(this.state.goodsRender)
			}
			if (this.state.goodsRender.length > 0 ) {
				return this.state.goodsRender.map(function(goodsItem, elem) {
					return (
				...
				return (
					<div style = { styles.noSearch } >
						<div style = { styles.searchbox } >
							<img style = { styles.searchimg } src = "common/images/ico_search@2x.png" />
						</div>
						<p style = { styles.searchtip } >没有搜索到相关商品</p>
					</div>
				)
			}
		};
...
function mapStateToProps (state) {
	...
	return {
		...
		sendMessage,
		cartReducer : searchReducer.cartCount,
	}
}
...
function mapDispatchToProps (dispatch) {
	return {
		...
		resetSearchReduces : bindActionCreators(resetSearchReduces,dispatch)
	}
}
