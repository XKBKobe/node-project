...
import {getCookie, COOKIE_KEY_LOGINSTATUS} from '../../common/config/cookieOperate';
...
class CartPage extends BaseComponent {
	constructor(props) {
		...
		this.gotoMakeSureOrder = this.gotoMakeSureOrder.bind(this);
		...
		if (0 !== this.props.shoppingCartState.shoppingCartData.data.checkedGood) {
			...
			if (warehouseExceedPrice) {
				...
				return (
					<div
						style = {styles.settleWrap}
						onClick = {() => this.gotoMakeSureOrder()}
					>
						<p style = {styles.settleInfo}>{STRING_RESOURCE.settlement}</p>
					</div>
				);
	...
	footerBarRender() {
		if (0 !== this.props.shoppingCartState.shoppingCartData.data.whGoods.length) {
			...
			return (
				<div style = {styles.cartPageFooter}>
