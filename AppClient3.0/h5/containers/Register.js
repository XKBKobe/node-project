...
class Register extends Component {
		...
		return (
			<div style = { styles.login } >
				<div style = { styles.header } >
					<p style = { styles.headerTip } >快速注册</p>
				</div>
				<div style = { styles.loginIn } >
					...
					<img
						...
						src = "common/images/ic_clear_input@3x.png"
					/>
				...
				<div style = { styles.loginIn } >
					<input style = { styles.loginInput } value = { this.state.pwd } ref = "pwd" onChange = { () => this.pwdChange(this.refs.pwd,1) } type = "password" placeholder = "请输入密码" />
	...
	getVerify (){			//获取验证码
		...
		if (self.state.verifyFlag) {
			...
			if (!/^1[3|4|5|7|8][0-9]\d{4,8}$/.test(phoneVal)) {
				...
				request("http://api3.bestinfoods.com/user/message/login_code","POST",obj)
				.then(data => {
					if (data.errorcode == 0) {
						var leftTime = 60;
	...
	loginHandle (){
		...
		if (!checked) {
			...
			request("http://api3.bestinfoods.com/user/password/register","POST",obj)
			.then(data => {
				if (data.errorcode == 0) {
