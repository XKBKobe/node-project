...
export function changeOrderState(state = orderState, action){
  switch (action.type) {
		case Types.MYALLORDER :
			return merge({}, state, {
				orderState : '0',
        selected : [true,false,false,false,false],
        message : state.myAllOrder,
			})
			break;
        ...
        message : state.waitReceiveOrder,
			})
			break;
        ...
        message : state.waitCommentOrder,
			})
			break;
		default:
			return merge({}, state, {
...
export function getDetailData(state=orderDetailState, action){
  switch (action.type) {
    case Types.ORDERDETAILDATA:
      return {
