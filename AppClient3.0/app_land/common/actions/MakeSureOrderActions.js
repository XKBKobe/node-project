...
import {hashHistory, browserHistory, Link} from 'react-native-router-flux';

export function queryShoppingCartOrderData() {
    var orderRequest = requestData('http://api3.bestinfoods.com/order/get/prepaywhenoncart', 'POST');
...
export function queryImmediatelyBuyOrderData(goodsId, amount) {
    ...
    var orderRequest = requestData('http://api3.bestinfoods.com/order/get/directpurchase', 'POST', 'goods=' + param);
    var addressRequest = requestData('http://api3.bestinfoods.com/user/address/list', 'POST');
...
export function payEventHandle(goodsParam, addressIdParam, totalSalePriceParam) {
    ...
    return ((dispatch) => {
        requestData('http://api3.bestinfoods.com/order/post/buyoncart', 'POST',
